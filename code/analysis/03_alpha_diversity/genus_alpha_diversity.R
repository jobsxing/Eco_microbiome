...
# R1Q8: repeated-measures-corrected genus-level alpha-diversity comparisons.
...
genus_relative[genus_relative < 1e-5] <- 0
genus_relative <- genus_relative[, colMeans(genus_relative > 0) >= 0.10, drop = FALSE]
...
# Exhaustive within-panel comparisons for Fig. 2A and Fig. S3. Comparisons
# across Human, Animal, and Environment are not scientifically meaningful for
# this display and are therefore not included in the bracket set.
panel_sources <- list(
  Human = source_levels[1:4],
  Animal = source_levels[5:7],
...
write_csv(global, file.path(out_dir, "sup_table_R1Q8_global_repeated_measures_tests.csv"))
write_csv(focal, file.path(out_dir, "sup_table_R1Q8_all_pairwise_contrasts.csv"))
...
fig2a_style <- ggplot(raw_long, aes(Display_Source, Value)) +
  ...
  labs(
    x = NULL, y = NULL, title = "Genus level",
...
ggsave(
  file.path(out_dir, "sup_fig_R1Q8_all_pairwise_q.pdf"), fig2a_style,
  width = 12.2, height = 10.2, units = "in", device = cairo_pdf
)
ggsave(
  file.path(out_dir, "sup_fig_R1Q8_all_pairwise_q.png"), fig2a_style,
...
fig2a_revised <- filter_plot_metric(fig2a_style, "Shannon")
figs3_revised <- filter_plot_metric(fig2a_style, "Richness")
ggsave(file.path(out_dir, "Fig2A_all_pairwise_q.pdf"), fig2a_revised,
       width = 12.2, height = 4.25, units = "in", device = cairo_pdf)
ggsave(file.path(out_dir, "Fig2A_all_pairwise_q.png"), fig2a_revised,
       width = 12.2, height = 4.25, units = "in", dpi = 300, bg = "white")
ggsave(file.path(out_dir, "FigS3_all_pairwise_q.pdf"), figs3_revised,
       width = 12.2, height = 4.25, units = "in", device = cairo_pdf)
ggsave(file.path(out_dir, "FigS3_all_pairwise_q.png"), figs3_revised,
       width = 12.2, height = 4.25, units = "in", dpi = 300, bg = "white")

message("R1Q8 cluster-robust repeated-measures analysis completed")
